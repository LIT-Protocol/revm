...
use crate::{
    ...
    PrecompileWithAddress, Vec,
};

pub const EC_OPERATION: PrecompileWithAddress = PrecompileWithAddress(
    crate::u64_to_address(301),
    Precompile::Standard(|input, gas_limit| ec_operation(input, gas_limit)),
);
...
impl TryFrom<u8> for EcOperation {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            _ => Err(Self::Error::other("Invalid EC Ops operation")),
        }
    }
}
...
impl FromStr for EcOperation {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            _ => Err(Self::Err::other("Invalid EC Ops operation")),
        }
    }
}
...
fn ec_operation(input: &Bytes, gas_limit: u64) -> PrecompileResult {
    if input.is_empty() {
        return Err(PrecompileError::other(
            "Invalid EC Ops operation in ec_operation.  Input is empty.",
        )
        .into());
    }
    ...
    Err(PrecompileError::other(
        "Invalid EC Ops operation in ec_operation.  At end of ec_operation()",
    )
    .into())
}
...
trait EcOps {
    fn handle(&self, data: &[u8], gas_limit: u64) -> PrecompileResult {
        ...
        while i < data.len() {
            if i + 32 > data.len() {
                return Err(
                    PrecompileError::other("Invalid EC Ops Curve in top of handle()").into(),
                );
            }
            match &data[i..i + 32] {
                CURVE_NAME_SECP256K1 => {
                    let result = self.secp256k1(&data[i + 32..])?;
                    return Ok(PrecompileOutput::new(gas_used, result.into()));
                }
        ...
        Err(PrecompileError::other("Invalid EC Ops Curve at end of handle()").into())
    }
    fn secp256k1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError>;
    fn prime256v1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError>;
    fn secp384r1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError>;
    fn curve25519(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError>;
    fn bls12381g1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError>;
    fn bls12381g2(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError>;
    fn bls12381gt(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError>;
    fn curve448(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError>;
    fn jubjub(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError>;
}

fn parse_hash(data: &[u8]) -> Result<(&[u8], Box<dyn SchnorrChallenge>), PrecompileError> {
    match &data[..32] {
        ...
        _ => Err(PrecompileError::other("Invalid EC Ops Hash in parse_hash()").into()),
    }
}
...
pub(crate) fn secp256k1_points(
    data: &[u8],
    point_cnt: usize,
) -> Result<(&[u8], Vec<k256::ProjectivePoint>), PrecompileError> {
    if 33 * point_cnt > data.len() {
        return Err(PrecompileError::other(format!(
            "Invalid EC OPs Point - {}:{} - data.len() is {} but should be at least {} bytes",
            file!(),
            line!(),
            data.len(),
            33 * point_cnt
    ...
    let points =
        extract_points::<k256::Secp256k1>(&data[..33 * point_cnt], point_cnt).map_err(|_| {
            PrecompileError::other(format!("Invalid EC OPs Point - {}:{}", file!(), line!())).into()
        })?;
    if points.len() != point_cnt {
        return Err(PrecompileError::other(format!(
            "Invalid EC OPs Point - {}:{}",
            file!(),
            line!()
        ))
        .into());
    }
    Ok((&data[33 * point_cnt..], points))
}

fn secp256k1_scalars(
    data: &[u8],
    scalar_cnt: usize,
) -> Result<(&[u8], Vec<k256::Scalar>), PrecompileError> {
    if 32 * scalar_cnt > data.len() {
        return Err(PrecompileError::other("Invalid EC Ops Scalar").into());
    }
    ...
    for i in 0..scalar_cnt {
        ...
        let scalar = Option::<k256::Scalar>::from(k256::Scalar::from_repr(repr))
            .ok_or(PrecompileError::other("Invalid EC Ops Scalar").into())?;
        scalars.push(scalar);
    }
...
pub(crate) fn prime256v1_points(
    data: &[u8],
    point_cnt: usize,
) -> Result<(&[u8], Vec<p256::ProjectivePoint>), PrecompileError> {
    ...
    let points =
        extract_points::<p256::NistP256>(&data[..64 * point_cnt], point_cnt).map_err(|_| {
            PrecompileError::other(format!("Invalid EC OPs Point - {}:{}", file!(), line!())).into()
        })?;
    if points.len() != point_cnt {
        return Err(PrecompileError::other(format!(
...
fn prime256v1_scalars(
    data: &[u8],
    scalar_cnt: usize,
) -> Result<(&[u8], Vec<p256::Scalar>), PrecompileError> {
    if 32 * scalar_cnt > data.len() {
        return Err(PrecompileError::other("Invalid EC Ops Scalar").into());
    }
    let mut scalars = Vec::with_capacity(scalar_cnt);
...
pub(crate) fn secp384r1_points(
    data: &[u8],
    point_cnt: usize,
) -> Result<(&[u8], Vec<p384::ProjectivePoint>), PrecompileError> {
    use elliptic_curve::sec1::FromEncodedPoint;

    if 96 * point_cnt > data.len() {
        return Err(PrecompileError::other("Invalid EC Ops Point in secp384r1_points()").into());
    }
        ...
        let point = match data[offset] {
            0x04 => {
                ...
                if offset + 97 > data.len() {
                    return Err(PrecompileError::other(
                        "Invalid EC Ops Point in secp384r1_points()",
                    )
                    .into());
                }
                let encoded_point =
                    ...
                    .map_err(|_| {
                        PrecompileError::other("Invalid EC Ops Point in secp384r1_points()").into()
                    })?;
            ...
            _ => {
                ...
                let encoded_point =
                    elliptic_curve::sec1::EncodedPoint::<p384::NistP384>::from_bytes(&tmp[..])
                        .map_err(|_| {
                            PrecompileError::other("Invalid EC Ops Point in secp384r1_points()")
                                .into()
                        })?;
                let point = p384::AffinePoint::from_encoded_point(&encoded_point)
                    .map(p384::ProjectivePoint::from);
...
fn secp384r1_scalars(
    data: &[u8],
    scalar_cnt: usize,
) -> Result<(&[u8], Vec<p384::Scalar>), PrecompileError> {
...
pub(crate) fn curve25519_points(
    data: &[u8],
    point_cnt: usize,
) -> Result<(&[u8], Vec<curve25519_dalek::EdwardsPoint>), PrecompileError> {
    if 64 * point_cnt > data.len() {
        return Err(PrecompileError::other(format!(
            "Invalid EC OPs Point - {}:{}",
            file!(),
    ...
    for i in 0..point_cnt {
        ...
        let point = compressed_point.decompress().ok_or(
            PrecompileError::other(format!("Invalid EC OPs Point - {}:{}", file!(), line!()))
                .into(),
        )?;
        points.push(point);
    }
...
pub(crate) fn ristretto25519_points(
    data: &[u8],
    point_cnt: usize,
) -> Result<(&[u8], Vec<curve25519_dalek::RistrettoPoint>), PrecompileError> {
    if 64 * point_cnt > data.len() {
        return Err(PrecompileError::other(format!(
            "Invalid EC OPs Point - {}:{}",
            file!(),
...
fn curve25519_scalars(
    data: &[u8],
    scalar_cnt: usize,
) -> Result<(&[u8], Vec<curve25519_dalek::Scalar>), PrecompileError> {
    if 32 * scalar_cnt > data.len() {
        return Err(PrecompileError::other("Invalid EC Ops Scalar").into());
    }
    let mut scalars = Vec::with_capacity(scalar_cnt);
...
pub(crate) fn bls12381g1_points(
    data: &[u8],
    point_cnt: usize,
) -> Result<(&[u8], Vec<blsful::inner_types::G1Projective>), PrecompileError> {
    ...
    for i in 0..point_cnt {
        ...
        .unwrap();
        let point = Option::<G1Projective>::from(G1Projective::from_uncompressed(&bytes)).ok_or(
            PrecompileError::other(format!("Invalid EC OPs Point - {}:{}", file!(), line!()))
                .into(),
        )?;
        points.push(point);
...
pub(crate) fn bls12381g2_points(
    data: &[u8],
    point_cnt: usize,
) -> Result<(&[u8], Vec<blsful::inner_types::G2Projective>), PrecompileError> {
    ...
    for i in 0..point_cnt {
        ...
        .unwrap();
        let point = Option::<G2Projective>::from(G2Projective::from_uncompressed(&bytes)).ok_or(
            PrecompileError::other(format!("Invalid EC OPs Point - {}:{}", file!(), line!()))
                .into(),
        )?;
        points.push(point);
...
fn bls12381gt_scalar(
    data: &[u8],
    cnt: usize,
) -> Result<(&[u8], Vec<blsful::inner_types::Gt>), PrecompileError> {
...
fn bls12381_scalars(
    data: &[u8],
    scalar_cnt: usize,
) -> Result<(&[u8], Vec<blsful::inner_types::Scalar>), PrecompileError> {
    if 32 * scalar_cnt > data.len() {
        return Err(PrecompileError::other("Invalid EC Ops Scalar").into());
    }
    let mut scalars = Vec::with_capacity(scalar_cnt);
...
pub(crate) fn curve448_points(
    data: &[u8],
    points_cnt: usize,
) -> Result<(&[u8], Vec<ed448_goldilocks_plus::EdwardsPoint>), PrecompileError> {
...
fn curve448_scalars(
    data: &[u8],
    scalar_cnt: usize,
) -> Result<(&[u8], Vec<ed448_goldilocks_plus::Scalar>), PrecompileError> {
...
pub(crate) fn jubjub_points(
    data: &[u8],
    point_cnt: usize,
) -> Result<(&[u8], Vec<jubjub::SubgroupPoint>), PrecompileError> {
    ...
    let mut points = Vec::with_capacity(point_cnt);
    for i in 0..point_cnt {
        let bytes = &data[(32 * i)..32 * (i + 1)].try_into().map_err(|_| {
            PrecompileError::other(format!("Invalid EC OPs Point - {}:{}", file!(), line!())).into()
        })?;
        let compressed_point = jubjub::SubgroupPoint::from_bytes(bytes);
        let point = Option::from(compressed_point).ok_or(
            PrecompileError::other(format!("Invalid EC OPs Point - {}:{}", file!(), line!()))
                .into(),
        )?;
        points.push(point);
...
fn jubjub_scalars(
    data: &[u8],
    scalar_cnt: usize,
) -> Result<(&[u8], Vec<jubjub::Scalar>), PrecompileError> {
    if 32 * scalar_cnt > data.len() {
        return Err(PrecompileError::other("Invalid EC Ops Scalar").into());
    }
    let mut scalars = Vec::with_capacity(scalar_cnt);
...
fn read_usizes(data: &[u8], cnt: usize) -> Result<(&[u8], Vec<usize>), PrecompileError> {
    if 32 * cnt > data.len() {
        return Err(PrecompileError::other("Invalid EC OPs size").into());
    }
    ...
    for i in 0..cnt {
        ...
        let length = length
            .to_usize()
            .ok_or(PrecompileError::other("Invalid EC OPs size").into())?;
        lengths.push(length);
    }
...
impl EcOps for EcPairing {
    fn secp256k1(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Ops Curve").into())
    }

    fn prime256v1(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Ops Curve").into())
    }

    fn secp384r1(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Ops Curve").into())
    }

    fn curve25519(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Ops Curve").into())
    }
    ...
    fn curve448(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Ops Curve").into())
    }

    fn jubjub(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Ops Curve").into())
    }
}
...
impl EcOps for ScalarInv {
    fn secp256k1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        let (_, scalars) = secp256k1_scalars(data, 1)?;
        let scalar = Option::<k256::Scalar>::from(scalars[0].invert())
            .ok_or(PrecompileError::other("Invalid EC Ops Scalar").into())?;
        Ok(scalar.to_bytes().to_vec())
    }
    ...
    fn prime256v1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        let (_, scalars) = prime256v1_scalars(data, 1)?;
        let scalar = Option::<p256::Scalar>::from(scalars[0].invert())
            .ok_or(PrecompileError::other("Invalid EC Ops Scalar").into())?;
        Ok(scalar.to_bytes().to_vec())
    }
    ...
    fn secp384r1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        let (_, scalars) = secp384r1_scalars(data, 1)?;
        let scalar = Option::<p384::Scalar>::from(scalars[0].invert())
            .ok_or(PrecompileError::other("Invalid EC Ops Scalar").into())?;
        Ok(scalar.to_bytes().to_vec())
    }
    ...
    fn jubjub(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        let (_, scalars) = jubjub_scalars(data, 1)?;
        let scalar = Option::<jubjub::Scalar>::from(scalars[0].invert())
            .ok_or(PrecompileError::other("Invalid EC Ops Scalar").into())?;
        Ok(scalar.to_bytes().to_vec())
    }
}
...
impl EcOps for ScalarSqrt {
    fn secp256k1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        ...
        if is_sqr.into() {
            ...
            Err(PrecompileError::other("Invalid EC Ops Scalar").into())
        }
    }
...
impl EcOps for EcdsaVerify {
    fn secp256k1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        ...
        let signature = k256::ecdsa::Signature::from_slice(&data[..64])
            .map_err(|_| PrecompileError::other("Invalid EC Ops signature").into())?;
    ...
    fn curve25519(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Op - not supported").into())
    }

    fn bls12381g1(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Op - not supported").into())
    }

    fn bls12381g2(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Op - not supported").into())
    }

    fn bls12381gt(&self, _data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        Err(PrecompileError::other("Invalid EC Op - not supported").into())
    }
...
impl EcOps for SchnorrVerify1 {
    fn secp256k1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        ...
        if data.len() < 64 {
            return Err(PrecompileError::other("Invalid EC Ops signature").into());
        }
        ...
        let s = k256::NonZeroScalar::try_from(&data[32..])
            .map_err(|_| PrecompileError::other("Invalid EC Ops Scalar").into())?;
        if s.is_zero().into() {
            return Err(PrecompileError::other("Invalid EC Ops Scalar").into());
        }
    ...
    fn prime256v1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        ...
        let s = p256::NonZeroScalar::try_from(&data[32..])
            .map_err(|_| PrecompileError::other("Invalid EC Ops Scalar").into())?;
        if s.is_zero().into() {
            return Err(PrecompileError::other("Invalid EC Ops Scalar").into());
        }
    ...
    fn secp384r1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        ...
        let s = p384::NonZeroScalar::try_from(&data[48..])
            .map_err(|_| PrecompileError::other("Invalid EC Ops Scalar").into())?;
        if s.is_zero().into() {
            return Err(PrecompileError::other("Invalid EC Ops Scalar").into());
        }
...
impl EcOps for BlsVerify {
    ...
    fn bls12381g1(&self, data: &[u8]) -> Result<Vec<u8>, PrecompileError> {
        ...
        if sig.verify(&pk, message).is_ok() {
            Ok(vec![1u8])
        } else {
            Ok(vec![0u8])
        }
    ...
    ecdsa::SignatureSize<C>: elliptic_curve::generic_array::ArrayLength<u8>,
{
    ...
    if *r == elliptic_curve::Scalar::<C>::reduce_bytes(&x) {
        ...
        Err(PrecompileError::other("Invalid EC Ops signature").into())
    }
}
