...
pub const DERIVE_CAIT_SITH_PUBKEY: PrecompileWithAddress = PrecompileWithAddress(
    crate::u64_to_address(245),
    Precompile::Standard(|input, gas_limit| derive_cait_sith_pubkey(input, gas_limit)),
);
...
fn derive_cait_sith_pubkey(input: &Bytes, gas_limit: u64) -> PrecompileResult {
    println!(
        "Lit Precompile: derive_cait_sith_pubkey - input.len() is {}",
        input.len()
    );
    ...
    for i in 0..input.len() {
        if let Ok(params) = DeriveParamCnt::try_from(&input[i..]) {
            println!("Lit Precompile: DeriveParamCnt: {:?}", params);
            let pk = params.derive_public_key();
            println!(
                "Lit Precompile: pk result from params.derive_public_key: {:?}",
                pk
            );
            return pk.map(|pk| PrecompileOutput::new(gas_used, pk.bytes));
        }
        if input.len() - i < MIN_LENGTH {
            break;
        }
    }
    Err(PrecompileError::Other("Invalid input".to_string()).into())
}
...
impl<'a> DeriveParamCnt<'a> {
    pub fn derive_public_key(&self) -> PrecompileResult {
    ...
    where
        ...
        D: HDDeriver,
        F: Fn(&[u8], usize) -> Result<(&[u8], Vec<B>), PrecompileError>,
        O: Fn(&B) -> Vec<u8>,
    {
        println!("Lit Precompile: compute_new_public_key");
        let (_, public_keys) = convert_points(self.buffer, self.public_key_count)
            .map_err(|err| PrecompileError::other(err.to_string()))?;
        println!("Lit Precompile: public_keys: {:?}", public_keys);
        let tweak = D::create(self.id, self.cxt);
        println!("Lit Precompile: tweak: {:?}", tweak);
        let pk = tweak.hd_derive_public_key(&public_keys);
        println!("Lit Precompile: pk: {:?}", pk);
        Ok(PrecompileOutput::new(0, out_point(&pk).into()))
    }
}
...
fn run_test_k256() {
    ...
    let res = derive_cait_sith_pubkey(&temp.into(), 1000000000000000000);
    assert!(res.is_ok());
}
